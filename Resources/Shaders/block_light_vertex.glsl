...
void main()
{
	...
	float x_half = (((specifications & (1 << 17)) == 0) ? half_pxl : one256th - half_pxl);
	float y_half = (((specifications & (1 << 18)) == 0) ? half_pxl : one256th - half_pxl);
	...
	x_half = (((specifications & (1 << 17)) == 0) ? half_pxl : one16th - half_pxl);
	y_half = (((specifications & (1 << 18)) == 0) ? half_pxl : one16th - half_pxl);
	NumCoords = vec2(13 * one16th + x_half, (blockLight - 1) * one16th + y_half);
}
