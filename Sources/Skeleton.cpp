...
bool Skeleton::update( std::vector<t_shaderInputModel>& modArr, float deltaTime )
{
    if (!_chunk) {
...
void Skeleton::draw( std::vector<t_shaderInputModel>& arr )
{
...
// draw head
	...
	//bottom
	spec += 8;
	utils::shader::addQuads(arr, {p6, p7, p3, p2}, spec, 8, 8);
...
// draw torso
	...
	// addQuads(arr, {p0, p1, p2, p3}, spec, 8, 4); // not drawn
	// down
	...
	utils::shader::addQuads(arr, {p2, p3, p4, p5}, spec, 8, 12); // recto
	utils::shader::addQuads(arr, {p3, p2, p5, p4}, spec, 8, 12, true); // verso
	// back 1076
	spec += 12;
	utils::shader::addQuads(arr, {p1, p0, p7, p6}, spec, 8, 12); // recto
	utils::shader::addQuads(arr, {p0, p1, p6, p7}, spec, 8, 12, true); // verso
	// right 0264
...
// draw right arm
	...
	utils::shader::addQuads(arr, {p0, p1, p2, p3}, spec, 2, 2);
	// down
	...
	spec += 2;
	utils::shader::addQuads(arr, {p4, p5, p6, p7}, spec, 2, 2);
	// front 2345
	spec = (2 << 19) + 42 + (18 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p2, p3, p4, p5}, spec, 2, 12);
	// back 1076
	spec += 4;
	utils::shader::addQuads(arr, {p1, p0, p7, p6}, spec, 2, 12);
	// right 0264
	spec = (2 << 19) + 40 + (18 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p0, p2, p6, p4}, spec, 2, 12);
	// left 3157
	spec += 4;
	utils::shader::addQuads(arr, {p3, p1, p5, p7}, spec, 2, 12);
