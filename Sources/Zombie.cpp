...
/**
 * @brief called when zombie goes from one block to another, used to update its path and state
 */
void Zombie::updatePath( void )
{
	...
	} else if (glm::distance(_position, _player->getPos()) < 32.0f) {
...
// draw head
	...
	int modelAltas = (settings::consts::shader::texture::zombie << 12);
...
// draw torso
	...
	utils::shader::addQuads(arr, {p0, p1, p2, p3}, spec, 8, 4 << 6);
	// down
	...
	utils::shader::addQuads(arr, {p2, p3, p4, p5}, spec, 8, 12 << 6);
	// back 1076
	spec += 12;
	utils::shader::addQuads(arr, {p1, p0, p7, p6}, spec, 8, 12 << 6);
	// right 0264
...
// draw right arm
	...
	spec = (2 << 19) + 44 + (16 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p0, p1, p2, p3}, spec, 4, 4 << 6);
	// down
	...
	spec += 4;
	utils::shader::addQuads(arr, {p4, p5, p6, p7}, spec, 4, 4 << 6);
	// front 2345
	spec = (2 << 19) + 44 + (20 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p2, p3, p4, p5}, spec, 4, 12 << 6);
	// back 1076
	spec += 8;
	utils::shader::addQuads(arr, {p1, p0, p7, p6}, spec, 4, 12 << 6);
	// right 0264
	spec = (2 << 19) + 40 + (20 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p0, p2, p6, p4}, spec, 4, 12 << 6);
	// left 3157
	spec += 8;
	utils::shader::addQuads(arr, {p3, p1, p5, p7}, spec, 4, 12 << 6);
...
// draw left leg // whose movement is in synch with right arm
	...
	spec = (2 << 19) + 4 + (16 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p0, p1, p2, p3}, spec, 4, 4 << 6);
	// down
	...
	utils::shader::addQuads(arr, {p4, p5, p6, p7}, spec, 4, 4 << 6);
	// front 2345
	spec = (2 << 19) + 4 + (20 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p2, p3, p4, p5}, spec, 4, 12 << 6);
	// back 1076
	spec += 8;
	utils::shader::addQuads(arr, {p1, p0, p7, p6}, spec, 4, 12 << 6);
	// right 0264
	spec = (2 << 19) + 0 + (20 << 6) + (itemLight << 24) + hurtColor + modelAltas;
	utils::shader::addQuads(arr, {p0, p2, p6, p4}, spec, 4, 12 << 6);
	// left 3157
	spec += 8;
	utils::shader::addQuads(arr, {p3, p1, p5, p7}, spec, 4, 12 << 6);
