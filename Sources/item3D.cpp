...
namespace EXTRUSION {
	...
	bool drawItem3D( std::vector<t_shaderInput> &arr, int type, int light, glm::vec3 pos, glm::vec3 front, glm::vec3 right, glm::vec3 up, float size )
	{
		...
		// back face
		spec += 16;
		utils::shader::addQuads(arr, {pos + front * size * one16th + right * size, pos + front * size * one16th, pos + front * size * one16th + (right - up) * size, pos + front * size * one16th - up * size}, spec + (1 << 17), -16 - (2 << 17), 16, 0, 8);
		// up faces
