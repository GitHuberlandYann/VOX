...
void Block::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
	...
	t_shaderInputItem v0 = {spec, {x, y}};
	t_shaderInputItem v1 = {spec + (1 << 4), {x + width * gui_size, y}};
	t_shaderInputItem v2 = {spec + (1 << 12), {x, y + width * gui_size}};
	t_shaderInputItem v3 = {spec + (1 << 4) + (1 << 12), {x + width * gui_size, y + width * gui_size}};
...
void Cube::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
	...
	t_shaderInputItem v0 = {spec, {x, y + width * gui_size * 81.25f / 362.5f}};
	t_shaderInputItem v1 = {spec + (1 << 4), {x + (width - 2) * 0.5f * gui_size, y}};
	t_shaderInputItem v2 = {spec + (1 << 12), {x + (width - 2) * 0.5f * gui_size, y + width * gui_size * 162.5f / 362.5f}};
	t_shaderInputItem v3 = {spec + (1 << 4) + (1 << 12), {x + (width - 2) * gui_size, y + width * gui_size * 81.25f / 362.5f}};
	ui->addFace(v0, v1, v2, v3, alien, movement);
	// left face
...
void StairsBottom::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
	...
	t_shaderInputItem v0 = {spec, {x, y + width * gui_size * 81.25f / 362.5f}};
	t_shaderInputItem v1 = {spec + (1 << 4), {x + (width - 2) * 0.5f * gui_size, y}};
	t_shaderInputItem v2 = {spec + (1 << 12), {x + (width - 2) * 0.25f * gui_size, y + width * gui_size * 121.875f / 362.5f}};
	t_shaderInputItem v3 = {spec + (1 << 4) + (1 << 12), {x + (width - 2) * 0.75f * gui_size, y + width * gui_size * 40.625f / 362.5f}};
	ui->addFace(v0, v1, v2, v3, alien, movement);
	// top of first step
...
void Fence::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
	...
	// top of left pole
	t_shaderInputItem v0 = {spec + 6, {x + width * gui_size * 3.0f * one16th, y + width * gui_size * 3.66f * one16th}};
	t_shaderInputItem v1 = {spec + 10, {x + width * gui_size * 5.0f * one16th, y + width * gui_size * 3.0f * one16th}};
	t_shaderInputItem v2 = {spec + 6 + (4 << 8), {x + width * gui_size * 5.0f * one16th, y + width * gui_size * 4.33f * one16th}};
	t_shaderInputItem v3 = {spec + 10 + (4 << 8), {x + width * gui_size * 7.0f * one16th, y + width * gui_size * 3.66f * one16th}};
	ui->addFace(v0, v1, v2, v3, alien, movement);
	// top of right pole
...
void Button::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
	...
	t_shaderInputItem v0 = {spec, {x, y + width * gui_size * 81.25f / 362.5f}};
	t_shaderInputItem v1 = {spec + (1 << 4), {x + (width - 2) * 0.5f * gui_size, y}};
	t_shaderInputItem v2 = {spec + (1 << 12), {x + (width - 2) * gui_size * 40.5125f / 362.5f, y + width * gui_size * 121.7125f / 362.5f}};
	t_shaderInputItem v3 = {spec + (1 << 4) + (1 << 12), {x + (width - 2) * gui_size * 203.0125f / 362.5f, y + width * gui_size * 40.5125f / 362.5f}};
	ui->addFace(v0, v1, v2, v3, alien, movement);
	// left face
	spec -= (5 << 24);
	v0 = {spec, {x, y + width * gui_size * 81.25f / 362.5f}};
...
void Chest::addItem( UI *ui, int x, int y, int gui_size, int width, int depth, bool alien, bool movement ) const
{
    ...
    t_shaderInputItem v0 = {spec + 1, {x, y + width * gui_size * 81.25f / 362.5f}};
    t_shaderInputItem v1 = {spec + width, {x + (width - 2) * 0.5f * gui_size, y}};
    t_shaderInputItem v2 = {spec + 1 + (14 << 8), {x + (width - 2) * 0.5f * gui_size, y + width * gui_size * 162.5f / 362.5f}};
    t_shaderInputItem v3 = {spec + width + (14 << 8), {x + (width - 2) * gui_size, y + width * gui_size * 81.25f / 362.5f}};
    ui->addFace(v0, v1, v2, v3, alien, movement);
    // left face
    spec -= (5 << 24) + (2 << 4);
