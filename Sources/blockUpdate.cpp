...
void Chunk::add_block( bool useInventory, glm::ivec3 pos, int block_value, int previous )
{
	...
	for (int index = 0; index < 6; index++) {
		...
		if (s_blocks[adj & 0xFF]->transparent) { // update light status around placed block
			...
			setBlockAt(adj | blocks::NOTVISIBLE, pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
		}
		if ((adj & 0xFF) == blocks::OAK_FENCE || (adj & 0xFF) == blocks::GLASS_PANE) {
			switch (index) {
				case face_dir::MINUSX:
					setBlockAt(adj | (FENCE::PX << 12), pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
					break ;
				case face_dir::PLUSX:
					setBlockAt(adj | (FENCE::MX << 12), pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
					break ;
				case face_dir::MINUSY:
					setBlockAt(adj | (FENCE::PY << 12), pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
					break ;
				case face_dir::PLUSY:
					setBlockAt(adj | (FENCE::MY << 12), pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
					break ;
			}
		}
		if ((adj & 0xFF) == blocks::REDSTONE_DUST && (delta.z == -1 || type == blocks::TARGET)) {
			connectRedstoneDust(pos + delta, adj, false);
			setBlockAt(adj, pos.x + delta.x, pos.y + delta.y, pos.z + delta.z);
		}
	}*/
}
