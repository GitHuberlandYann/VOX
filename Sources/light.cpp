			...
			for (int level = 0; level < WORLD_HEIGHT; level++) {
				...
				if (type >= blocks::WHEAT_CROP && type <= blocks::WHEAT_CROP7) {
					...
					if (type == blocks::OAK_SLAB_BOTTOM || type == blocks::OAK_SLAB_TOP) {
						...
						if (visible_face(type, getBlockAt(row - 1, col, level, true), face_dir::MINUSX)) {
							...
							switch (orientation) {
								case face_dir::MINUSX:
									...
									break ;
								case face_dir::PLUSY:
									v0 = {spec + (shade << 22) + XTEX + YTEX, p4};
									v1 = {spec + (shade << 22) + YTEX, p0};
									v2 = {spec + (shade << 22) + XTEX, p6};
									v3 = {spec + (shade << 22), p2};
									break ;
								default:
									v0 = {spec + (shade << 22) + YTEX, p4};
									v1 = {spec + (shade << 22) + XTEX + YTEX, p0};
									v2 = {spec + (shade << 22), p6};
						...
						if (visible_face(type, getBlockAt(row + 1, col, level, true), face_dir::PLUSX)) {
							...
							switch (orientation) {
								case face_dir::PLUSX:
									...
									break ;
								case face_dir::MINUSY:
									v0 = {spec + (shade << 22) + XTEX + YTEX, p1};
									v1 = {spec + (shade << 22) + YTEX, p5};
									v2 = {spec + (shade << 22) + XTEX, p3};
									v3 = {spec + (shade << 22), p7};
									break ;
								default:
									v0 = {spec + (shade << 22) + YTEX, p1};
									v1 = {spec + (shade << 22) + XTEX + YTEX, p5};
									v2 = {spec + (shade << 22), p3};
						...
						if (visible_face(type, getBlockAt(row, col - 1, level, true), face_dir::MINUSY)) {
							...
							switch (orientation) {
								case face_dir::MINUSY:
									...
									break ;
								case face_dir::MINUSX:
									v0 = {spec + (shade << 22) + XTEX + YTEX, p0};
									v1 = {spec + (shade << 22) + YTEX, p1};
									v2 = {spec + (shade << 22) + XTEX, p2};
									v3 = {spec + (shade << 22), p3};
									break ;
								default:
									v0 = {spec + (shade << 22) + YTEX, p0};
									v1 = {spec + (shade << 22) + XTEX + YTEX, p1};
									v2 = {spec + (shade << 22), p2};
						...
						if (visible_face(type, getBlockAt(row, col + 1, level, true), face_dir::PLUSY)) {
							...
							switch (orientation) {
								case face_dir::PLUSY:
									...
									break ;
								case face_dir::PLUSX:
									v0 = {spec + (shade << 22) + XTEX + YTEX, p5};
									v1 = {spec + (shade << 22) + YTEX, p4};
									v2 = {spec + (shade << 22) + XTEX, p7};
									v3 = {spec + (shade << 22), p6};
									break ;
								default:
									v0 = {spec + (shade << 22) + YTEX, p5};
									v1 = {spec + (shade << 22) + XTEX + YTEX, p4};
									v2 = {spec + (shade << 22), p7};
						...
						if (visible_face(type, getBlockAt(row, col, level - 1, true), face_dir::MINUSZ)) {
							int spec = s_blocks[type]->texX(face_dir::MINUSZ, orientation) + (s_blocks[type]->texY(face_dir::MINUSZ, orientation) << 4) + (0 << 19);
							int faceLight = computeLight(row, col, level - 1);
							int shade = 0;
							spec += (faceLight << 24);
							switch (orientation) {
